...
最开始构建索引时，直接调用 GitHub API 抓取所有分类。结果跑了两三个分类就触发 rate limit（403），整个构建过程直接断掉。

**解决**：给 API 请求加了超时和重试，同时用 `sys.stdout.write(".")` 显示进度，避免"看起来卡住了"的错觉。后来也支持了增量同步，不用每次都全量构建。

### 踩坑 2：原始菜谱没有分量

HowToCook 的菜谱很多没有精确用量，比如"蒜蓉虾"只写了"海虾、蒜蓉酱、食用油、生抽"，没有克数。展示出来就是一串光秃秃的食材名。

**解决**：建立了一个常见食材默认用量表（`DEFAULT_AMOUNTS`），根据用餐人数自动缩放。比如 2 人份海虾推荐 200g，4 人份就自动翻倍到 400g。同时标注"参考估算值"，让用户心里有数。
...
"番茄"和"西红柿"、"马铃薯"和"土豆"是同一个东西，但字符串完全匹配不上。

**解决**：建立了一个食材别名映射表（`ingredient_aliases.json`），做了归一化处理：
...
一开始的购物清单就是平铺直叙地列出所有食材，像这样：

```
- 培根 - 手抓饼皮 - 油麦菜 - 海虾 - 甘竹牌鲮鱼罐头 - 生抽 - 白糖 - 芝士
```

用户说"能不能分类一下"。

**解决**：加了食材分类系统，按"海鲜水产、肉禽蛋、蔬菜菌菇、豆制品、调料、辅料"等类别分行展示，逛超市时不用来回跑：
...
一开始脚本、数据、技能描述文件散落各处。用户提醒"技能的脚本应该放在技能目录下"，于是重构了目录结构，把所有内容收拢到 `skills/how-to-cook-skill/` 下，变成自包含的独立技能包。

### 意外收获：19 个单元测试

在开发过程中坚持了 TDD（测试驱动开发），给营养计算、食材匹配、推荐引擎都写了测试。总共 19 个测试，每次修改后跑一遍，心里踏实。这也是为什么目录调整了那么多次还能快速验证功能正常的原因。
